...
export default function Page() {
  ...
  const [isNavigating, setIsNavigating] = useState(false); // Flag to handle navigation state
  ...
  const handleItemClick = (index: number) => {
    console.log("Active index:", index); // Log the active index
    setActiveIndex(index); // Update the active index state
    setIsNavigating(true); // Set the navigating flag to true
    setTimeout(() => {
      // Navigate after the animation completes (delay of 1.5s)
      setIsNavigating(false);
    }, 1500); // Wait for animation duration (1.5 seconds)
  };
